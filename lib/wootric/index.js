var integration = require('analytics.js-integration');

/**
 * Expose `Wootric` integration.
 */

var Wootric = module.exports = integration('Wootric')
  .assumesPageview()
    .option('account_token', '')
    .global('wootricSettings')
    .global('wootric_survey_immediately')
    .global('wootric')
    //The integration object
    .global('_wootric')
    .tag('library', '<script src="http://d27j601g4x0gd5.cloudfront.net/segmentioSnippet.js"></script>')
    //.tag('library', '<script type=\"text\/javascript\">\/*{literal}<![CDATA[*\/ window.lightningjs||function(c){function g(b,d){d&&(d+=(\/\\?\/.test(d)?\"&\":\"?\")+\"lv=1\");c[b]||function(){var i=window,h=document,j=b,g=h.location.protocol,l=\"load\",k=0;(function(){function b(){a.P(l);a.w=1;c[j](\"_load\")}c[j]=function(){function m(){m.id=e;return c[j].apply(m,arguments)}var b,e=++k;b=this&&this!=i?this.id||0:0;(a.s=a.s||[]).push([e,b,arguments]);m.then=function(b,c,h){var d=a.fh[e]=a.fh[e]||[],j=a.eh[e]=a.eh[e]||[],f=a.ph[e]=a.ph[e]||[];b&&d.push(b);c&&j.push(c);h&&f.push(h);return m}; return m};var a=c[j]._={};a.fh={};a.eh={};a.ph={};a.l=d?d.replace(\/^\\\/\\\/\/,(g==\"https:\"?g:\"http:\")+\"\/\/\"):d;a.p={0:+new Date};a.P=function(b){a.p[b]=new Date-a.p[0]};a.w&&b();i.addEventListener?i.addEventListener(l,b,!1):i.attachEvent(\"on\"+l,b);var q=function(){function b(){return[\"<head><\/head><\",c,\' onload=\"var d=\',n,\";d.getElementsByTagName(\'head\')[0].\",d,\"(d.\",g,\"(\'script\')).\",i,\"=\'\",a.l,\"\'\\\"><\/\",c,\">\"].join(\"\")}var c=\"body\",e=h[c];if(!e)return setTimeout(q,100);a.P(1);var d=\"appendChild\",g=\"createElement\", i=\"src\",k=h[g](\"div\"),l=k[d](h[g](\"div\")),f=h[g](\"iframe\"),n=\"document\",p;k.style.display=\"none\";e.insertBefore(k,e.firstChild).id=o+\"-\"+j;f.frameBorder=\"0\";f.id=o+\"-frame-\"+j;\/MSIE[ ]+6\/.test(navigator.userAgent)&&(f[i]=\"javascript:false\");f.allowTransparency=\"true\";l[d](f);try{f.contentWindow[n].open()}catch(s){a.domain=h.domain,p=\"javascript:var d=\"+n+\".open();d.domain=\'\"+h.domain+\"\';\",f[i]=p+\"void(0);\"}try{var r=f.contentWindow[n];r.write(b());r.close()}catch(t){f[i]=p+\'d.write(\"\'+b().replace(\/\"\/g, String.fromCharCode(92)+\'\"\')+\'\");d.close();\'}a.P(2)};a.l&&q()})()}();c[b].lv=\"1\";return c[b]}var o=\"lightningjs\",k=window[o]=g(o);k.require=g;k.modules=c}({}); window.wootric = lightningjs.require(\"wootric\", \"\/\/d27j601g4x0gd5.cloudfront.net\/beacon.js\"); \/*]]>{\/literal}*\/<\/script>')

/**
 * Initialize Wootric.
 *
 * @param {Facade} page
 */

Wootric.prototype.initialize = function(page){
  // TODO: fill in the logic that needs to be run for your integration to be
  // properly initialized. Most often that will include a call to `this.load()`
  // which comes next.
  //
  // Here's what a normal `initialize` method might look like:
  //
  //   window.__integration = {};
  //   window.__integration.apiKey = this.options.apiKey;
  //   this.load(this.ready);
  //
  // Once the integration is ready to receive calls to `identify()`, `track()` etc..
  // you must invoke `this.ready()`.
  // for example if the integration is ready once it's loaded, you can call `this.load(this.ready)`
  // if it's ready immediately after `initialize()` is called you can call `this.ready()` here.
  window._wootric = this;
  var self = this;
  window.wootricSettings = {
    response_callback: this.trackSurveyResponse,
    decline_callback: this.trackSurveyDecline,
    survey_callback: this.trackSurvey
  };

  window.wootricSettings.account_token = this.options.account_token;
  //Load our library, once it is loaded, we are ready
  this.load('library', function(){
    window.wootric("run");
    self.ready();
  });
};

// Track the survey result.  This is called back once the result is sent to Wootric
// by way of the wootricSettings.response_callback callback
Wootric.prototype.trackSurveyResponse = function(score, text){
  this.track('NPS Response', {
    email: window.wootricSettings.email,
    createdAt: window.wootricSettings.created_at,
    score: score,
    text: text
  })
}

// Track the survey result.  This is called back once the result is sent to Wootric
// by way of the wootricSettings.decline_callback callback
Wootric.prototype.trackSurveyDecline = function(){
  this.track('NPS Decline', {
    email: window.wootricSettings.email,
    createdAt: window.wootricSettings.created_at
  })
}

// Track the survey result.  This is called back once the result is sent to Wootric
// by way of the wootricSettings.survey_callback callback
Wootric.prototype.trackSurvey = function(){
  this.track('NPS Survey', {
    email: window.wootricSettings.email,
    createdAt: window.wootricSettings.created_at
  })
}

/**
 * Has the Wootric library been loaded yet?
 *
 * @return {Boolean}
 */

Wootric.prototype.loaded = function(){
  // TODO: fill in the logic required to check if the library has already been
  //We are always ready since we are just setting a global variable in initialize
  return window.wootric;
};

/**
 * Identify a user.
 *
 * @param {Facade} identify
 */

Wootric.prototype.identify = function(identify){
  //console.log("calling identify")
  var traits = identify.traits();
  var email = traits.trait.email;
  var created_at = traits.trait.createdAt;
  var unixDt = Date.parse(created_at) || +created_at;
  if (unixDt) {
    window.wootricSettings.created_at = unixDt;
  }
  window.wootricSettings.email = email;
  //Set the rest of the traits as properties
  window.wootricSettings.properties = traits.trait;
  //remove the email and createdAt properties
  delete window.wootricSettings.properties.email;
  delete window.wootricSettings.properties.createdAt;
  //Track page visit
  this.page();
  //console.log("wootricSettings", window.wootricSettings);
  };

//Store the last page name tracked
Wootric.lastPageTracked = null;

/**
 * Page.
 *
 * @param {Page} page
 */
Wootric.prototype.page = function(page){
  //console.log("calling page");
  var opts = this.options;
  //console.log(opts);
  if (opts.account_token) {
    //Only track page if we haven't already tracked it
    if (this.lastPageTracked !== window.location) {
      this.lastPageTracked = window.location;
      this.createTrackingPixel(window.location, this.options.account_token, window.wootricSettings.email, window.wootricSettings.created_at);
    }
  }
}

Wootric.prototype.createTrackingPixel = function(url, account_token, email, created_at){
  var i=new Image;
  i.src="//d8myem934l1zi.cloudfront.net/pixel.gif?account_token="+account_token+"&email="+encodeURIComponent(email)+"&created_at="+created_at+"&url="+encodeURIComponent(window.location)+"&random="+Math.random()
}