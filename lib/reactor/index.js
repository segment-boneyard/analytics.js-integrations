...
Reactor.prototype.initialize = function(page){
    (function(window, doc, reactor_name){
        window.ReactorAmObject = reactor_name;
        window[reactor_name] = window[reactor_name] || function(){
            (window[reactor_name].q = window[reactor_name].q || [])
                .push(arguments)
        };
    ...
    for (var prop_name in segment_properties) {
        ...
        switch (typeof prop_value) {
            case "string":
                ...
                break;
            case "number":
                reactor_prop_name += '_f';
                break;
            case "boolean":
                reactor_prop_name += '_b';
                prop_value = 0 + prop_value;
                break;
            case "object":
                if (prop_value === null) {
                    reactor_prop_name += '_s';
